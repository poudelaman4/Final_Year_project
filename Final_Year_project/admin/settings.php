...
$username_db = "";
$role_db = "";
$theme_preference_db = "light"; // Default
$items_per_page_db = 10;      // Default

$update_profile_success_message = "";
$update_profile_error_message = "";
$change_password_success_message = "";
$change_password_error_message = "";
$update_interface_success_message = "";
$update_interface_error_message = "";


// --- FETCH CURRENT ADMIN'S DATA ---
// Now fetching theme and items_per_page as well
$sqlFetchAdmin = "SELECT full_name, username, role, theme_preference, items_per_page FROM staff WHERE staff_id = ?";
if ($stmtFetch = mysqli_prepare($link, $sqlFetchAdmin)) {
    ...
    if (mysqli_stmt_execute($stmtFetch)) {
        ...
        if ($admin_data = mysqli_fetch_assoc($result)) {
            ...
            // Use fetched preferences if they exist and are valid
            $theme_preference_db = !empty($admin_data['theme_preference']) ? $admin_data['theme_preference'] : 'light';
            $items_per_page_db = !empty($admin_data['items_per_page']) ? (int)$admin_data['items_per_page'] : 10;
        } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    elseif (isset($_POST['update_interface_settings'])) {
        $new_theme = isset($_POST['theme_preference']) && $_POST['theme_preference'] === 'dark' ? 'dark' : 'light';
        $new_items_per_page = filter_input(INPUT_POST, 'items_per_page', FILTER_VALIDATE_INT, ['options' => ['default' => 10, 'min_range' => 5, 'max_range' => 100]]); // Validate integer between 5-100

        // Update database
        $sqlUpdateInterface = "UPDATE staff SET theme_preference = ?, items_per_page = ? WHERE staff_id = ?";
        if ($stmtUpdateInterface = mysqli_prepare($link, $sqlUpdateInterface)) {
            mysqli_stmt_bind_param($stmtUpdateInterface, "sii", $new_theme, $new_items_per_page, $current_admin_id);
            if (mysqli_stmt_execute($stmtUpdateInterface)) {
                $update_interface_success_message = "Interface settings updated successfully!";
                // Update variables for form display
                $theme_preference_db = $new_theme;
                $items_per_page_db = $new_items_per_page;

                // --- Log activity ---
                 $log_desc_ui = "Admin '{$username_db}' (ID: {$current_admin_id}) updated interface settings (Theme: {$new_theme}, Items: {$new_items_per_page}).";
                 $sqlLogUI = "INSERT INTO activity_log (timestamp, activity_type, description, admin_id, related_id) VALUES (NOW(), 'interface_update', ?, ?, ?)";
                 if($stmtLogUI = mysqli_prepare($link, $sqlLogUI)){
                    mysqli_stmt_bind_param($stmtLogUI, "sii", $log_desc_ui, $current_admin_id, $current_admin_id);
                    mysqli_stmt_execute($stmtLogUI);
                    mysqli_stmt_close($stmtLogUI);
                 }
                 // --- End log activity ---

            } else {
                $update_interface_error_message = "Error updating interface settings: " . mysqli_stmt_error($stmtUpdateInterface);
                error_log("DB Execute Error (settings.php - update interface): " . mysqli_stmt_error($stmtUpdateInterface));
            }
            mysqli_stmt_close($stmtUpdateInterface);
        } else {
            $update_interface_error_message = "Database error preparing interface settings update: " . mysqli_error($link);
            error_log("DB Prepare Error (settings.php - update interface): " . mysqli_error($link));
        }
    }
}
// --- END FORM PROCESSING ---
...
<!DOCTYPE html>
<html lang="en" class="<?php echo htmlspecialchars($theme_preference_db); ?>">
<head>
    <meta charset="UTF-8">
...
<body class="bg-gray-100 dark:bg-gray-900 font-sans">
    ...
    <div class="settings-container px-4">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-8 text-center">Settings</h1>

        <div class="settings-tabs">
            <button class="tab-button active" onclick="openTab(event, 'profile')">Profile</button>
            <button class="tab-button" onclick="openTab(event, 'interface')">Interface</button>
            <button class="tab-button" onclick="openTab(event, 'password')">Password</button>
            </div>

        <div id="profile" class="settings-content active">
            <section class="settings-section">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-white mb-6">My Profile Details</h2>
    ...
    <?php include '../includes/footer.php'; // Standard footer ?>
    <script>
        ...
        if (themeToggle) {
            themeToggle.addEventListener('change', function() {
                if (this.checked) {
                    ...
                    // Optionally store preference immediately in localStorage for instant feedback
                    localStorage.setItem('theme', 'dark');
                } else {
                    document.documentElement.classList.remove('dark');
                     localStorage.setItem('theme', 'light');
                }
                // Form submission will save it to the database server-side
            });
        }

         // Apply theme instantly based on localStorage on load (before DB value might be applied by PHP class)
         // This gives faster visual feedback but might flicker if localStorage mismatches DB on initial load
        // function applyInitialTheme() {
        //     const storedTheme = localStorage.getItem('theme');
        //     const dbTheme = document.documentElement.classList.contains('dark') ? 'dark' : 'light';
        ...
        //     }
        // }
        // applyInitialTheme(); // Call this early if needed
