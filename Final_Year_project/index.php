...
if (!$link || mysqli_connect_errno()) {
     ...
     // For now, let's set a flag or message
...
// Initialize session balance to N/A if not already set (prevents header error on first load)
if (!isset($_SESSION['student_balance'])) {
     $_SESSION['student_balance'] = 'N/A';
}
$student_balance = $_SESSION['student_balance']; // Use the session value by default

// Fetch student name and balance if logged in and DB connection is good
if (!$db_connection_error) {
    $student_id = (int)$_SESSION['student_id']; // Get student ID from session
    error_log("DEBUG index.php: student_id from session: " . $student_id);
        ...
        if (mysqli_stmt_execute($stmt_student)) {
            ...
            if (mysqli_num_rows($result_student) > 0) {
                 if ($student_info = mysqli_fetch_assoc($result_student)) {
                    ...
                    error_log("DEBUG index.php: Could not fetch student_info for ID: " . $student_id);
                 }
            } else {
                 error_log("Warning index.php: Student ID " . $student_id . " found in session but not in student table.");
                 // Optional: Log out user if their record is missing/invalid
            }
            mysqli_free_result($result_student); // Free result set
        } else {
             error_log("DB Error index.php: Could not execute student name fetch: " . mysqli_stmt_error($stmt_student));
        }
        mysqli_stmt_close($stmt_student);
    } else {
         error_log("DB Error index.php: Could not prepare student name fetch query: " . mysqli_error($link));
    }


    // --- BALANCE FETCHING AND STORING IN SESSION ---
    error_log("DEBUG index.php: Starting balance fetch for student_id: " . $student_id);
    $sql_balance = "SELECT current_balance FROM nfc_card WHERE student_id = ?";
     if ($stmt_balance = mysqli_prepare($link, $sql_balance)) {
        error_log("DEBUG index.php: Balance fetch query prepared.");
        mysqli_stmt_bind_param($stmt_balance, "i", $student_id);
        if (mysqli_stmt_execute($stmt_balance)) {
            error_log("DEBUG index.php: Balance fetch query executed successfully.");
            $result_balance = mysqli_stmt_get_result($stmt_balance);
            $num_balance_rows = mysqli_num_rows($result_balance);
            error_log("DEBUG index.php: Number of rows found in nfc_card for student_id " . $student_id . ": " . $num_balance_rows);

            if ($num_balance_rows > 0) { // Check if at least one row is found
                 if ($balance_info = mysqli_fetch_assoc($result_balance)) {
                    // Use the balance from the first found row
                    // Check if 'current_balance' key exists and is not null
                    if (isset($balance_info['current_balance'])) {
                         $fetched_balance = (float)$balance_info['current_balance'];
                         $_SESSION['student_balance'] = number_format($fetched_balance, 2); // Store formatted balance in session
                         $student_balance = $_SESSION['student_balance']; // Update local variable for this request
                         error_log("DEBUG index.php: Fetched balance value: " . $balance_info['current_balance'] . " -> formatted and stored in session as: " . $_SESSION['student_balance']);
                    } else {
                         error_log("Warning index.php: 'current_balance' key not found or is null in fetched balance row for student_id: " . $student_id);
                         $_SESSION['student_balance'] = "Data Error"; // Indicate data issue in session
                         $student_balance = $_SESSION['student_balance']; // Update local variable
                    }

                 } else {
                      error_log("DEBUG index.php: mysqli_fetch_assoc returned false for balance query after finding rows. Student ID: " . $student_id);
                      $_SESSION['student_balance'] = "Fetch Error"; // Indicate fetch problem in session
                      $student_balance = $_SESSION['student_balance']; // Update local variable
                 }

                 if ($num_balance_rows > 1) {
                     error_log("Warning index.php: Multiple NFC cards found for student_id: " . $student_id . ". Using the first one found.");
                     // The code already uses the first one, so no change needed unless you want to force an "Error" display here
                 }
            } else {
                 // No NFC card found, $_SESSION['student_balance'] remains "N/A" (initialized above)
                 error_log("Info index.php: No NFC card found for student_id: " . $student_id . ". Balance displayed as N/A.");
            }
            mysqli_free_result($result_balance); // Free result set
        } else {
             error_log("DB Error index.php: Could not execute balance fetch: " . mysqli_stmt_error($stmt_balance));
             $_SESSION['student_balance'] = "Error"; // Indicate DB error in session
             $student_balance = $_SESSION['student_balance']; // Update local variable
        }
        mysqli_stmt_close($stmt_balance);
     } else {
          error_log("DB Error index.php: Could not prepare balance fetch query: " . mysqli_error($link));
          $_SESSION['student_balance'] = "Error"; // Indicate DB error in session
          $student_balance = $_SESSION['student_balance']; // Update local variable
     }
     error_log("DEBUG index.php: Final student_balance value before display (from session): " . $student_balance);
    // --- END BALANCE FETCHING AND STORING IN SESSION ---

} else {
    // If DB connection failed, session balance remains what it was or is N/A
    // We already initialized/used session value, so no change needed here.
     error_log("DEBUG index.php: DB Connection failed, using session balance: " . $student_balance);
}
...
$is_student_logged_in = isset($_SESSION['student_id']) && !empty($_SESSION['student_id']) && !$db_connection_error; // Also consider DB connection
...
if (!$db_connection_error) {
    if ($stmt = mysqli_prepare($link, $sql)) {
        if (mysqli_stmt_execute($stmt)) {
            ...
            error_log("DB Error index.php: Error executing food fetch query: " . mysqli_stmt_error($stmt));
        }
        mysqli_stmt_close($stmt);
    } else {
        $no_items_message = "Error preparing food fetch query.";
        error_log("DB Error index.php: Error preparing food fetch query: " . mysqli_error($link));
    }
} else {
    $no_items_message = "Database connection failed. Cannot load menu.";
}
...
<body class="bg-gray-100 dark:bg-gray-900">
    <?php
    // --- Include Header HTML ---
    // The header now reads $student_balance from $_SESSION.
        ...
        <div class="container px-5 py-8 mx-auto">
            <div class="flex flex-wrap -m-4">
                ...
                if (!empty($food_items)) {
                    ...
                    foreach ($food_items as $food_item) {
                        ...
                        <div class="lg:w-1/4 md:w-1/2 p-4 w-full">
                            <div class="bg-white dark:bg-gray-700 rounded-lg overflow-hidden shadow-lg h-full flex flex-col">
                                ...
                                <div class="p-6 pt-0">
                                     <?php // $is_logged_in_for_button is now $is_student_logged_in ?>
                                     <button class="w-full text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded add-to-cart-btn" data-food-id="<?= htmlspecialchars($food_item['food_id'] ?? ''); ?>" <?= $is_student_logged_in ? '' : 'disabled'; ?>>+ Add</button>
                    ...
                    // Display the message if no food items were found or DB error occurred
                    ?>
                    <div class="w-full text-center py-8">
                        <p class="text-gray-500 dark:text-gray-400 text-lg"><?= htmlspecialchars($no_items_message ?? ''); ?></p>
                         <?php if ($db_connection_error): ?>
                             <p class="text-red-500 text-sm mt-2">Please check the database connection.</p>
                         <?php endif; ?>
                    </div>
                    <?php
                }
