...
// Include general header (adjust path if needed)
...
<head>
    ...
    <style>
        ...
        .order-table th, .order-table td { border: 1px solid #e2e8f0; padding: 0.75rem 1rem; text-align: left; font-size: 0.9rem; /* Smaller font for table */}
        .order-table th { background-color: #f7fafc; font-weight: 600; }
        .order-table tr:nth-child(even) { background-color: #edf2f7; }
        ...
        /* Dark Mode Styles */
        .dark .profile-section, .dark .orders-section { background-color: #1f2937; box-shadow: 0 4px 6px rgba(0,0,0,0.4); }
        ...
        .dark .nfc-status-Active { color: #34d399; } /* emerald-400 */
        .dark .nfc-status-Inactive, .dark .nfc-status-Lost, .dark .nfc-status-Blocked { color: #f87171; } /* red-400 */
        .dark .order-table { box-shadow: none; }
        .dark .order-table th, .dark .order-table td { border-color: #4a5568; }
        .dark .order-table th { background-color: #2d3748; color: #e2e8f0; }
        .dark .order-table tr:nth-child(even) { background-color: #2c3340; }
        .dark .order-table a { color: #63b3ed; }
        .dark .status-success { color: #68d391; }
        .dark .status-failed { color: #fc8181; }
        .dark .status-refunded { color: #f6ad55; }
        .dark .pagination a, .dark .pagination span { border-color: #4a5568; color: #cbd5e0; }
        .dark .pagination a:hover { background-color: #4a5568; }
        .dark .pagination .current-page { background-color: #63b3ed; border-color: #63b3ed; color: #1a202c; }
        .dark .pagination .disabled { color: #718096; }
        .dark .no-orders { color: #9ca3af; }
...
<body class="bg-gray-100 dark:bg-gray-900 font-sans">
    ...
    <div class="profile-container">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-8 text-center">My Profile & Order History</h1>
        ...
        <section class="orders-section">
            <h2 class="text-xl font-semibold text-gray-800 dark:text-white mb-6">Recent Orders</h2>
            ...
            <?php else: ?>
                <div class="overflow-x-auto">
                    <table class="min-w-full order-table">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Order ID</th>
                                ...
                                <th class="px-4 py-2 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Status</th>
                                <th class="px-4 py-2 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider"></th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <?php foreach ($orders as $order): ?>
                                <tr>
                                    <td class="px-4 py-2 whitespace-nowrap text-sm font-medium"><?php echo htmlspecialchars($order['formatted_id']); ?></td>
                                    ...
                                    <td class="px-4 py-2 whitespace-nowrap text-sm text-right">₹<?php echo number_format($order['total_amount'], 2); ?></td>
                                    ...
                                    <td class="px-4 py-2 whitespace-nowrap text-sm text-center">
                                        <a href="order_details.php?txn_id=<?php echo $order['txn_id']; ?>" title="View Details">Details</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                ...
                <nav class="pagination" aria-label="Order History Pagination">
                 ...
                 if($totalOrderRecords > $itemsPerPage) echo '<div class="text-center mt-4"><a href="order_history.php" class="text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300">View All Orders</a></div>';
                 ?>
                <?php endif; ?>
